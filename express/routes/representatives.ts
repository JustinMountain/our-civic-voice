...
router.get('/federal/:constituency', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const reps = await client.query(`SELECT honorific, first_name, last_name, constituency, party, province_territory, 'Federal' AS gov_level FROM federal_mps WHERE LOWER(constituency) = '${constituency}';`);
...
router.get('/ontario/:constituency', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const reps = await client.query(`SELECT honorific, first_name, last_name, constituency, party, 'Ontario' AS province_territory, 'Provincial' AS gov_level  FROM ontario_mpps WHERE LOWER(constituency) = '${constituency}';`);
